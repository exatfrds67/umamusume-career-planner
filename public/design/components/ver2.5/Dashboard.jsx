  ...
  return (
    <div>
      ...
      <Card style={{ padding:16 }}>
        <div style={{ fontSize:13, fontWeight:800, color:'#7C6FAB', marginBottom:12, letterSpacing:1, textTransform:'uppercase' }}>Quick Actions</div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:10 }}>
          {[
            { id:'training',      icon:'⚡', label:'Training',    sub:'Rec: Speed',       color:'#E879A0', bg:'#FDF2F8' },
            { id:'skills',        icon:'✨', label:'Skills',      sub:'2 hints ready',    color:'#F59E0B', bg:'#FFFBEB' },
            { id:'races',         icon:'🏆', label:'Race',        sub:'G2 Turn 52',       color:'#7C3AED', bg:'#F5F3FF' },
            { id:'skill-loadout', icon:'🎯', label:'Loadout',     sub:'Score: 88',        color:'#10B981', bg:'#F0FDF4' },
            { id:'ai-advisor',    icon:'🤖', label:'AI Advisor',  sub:'Ask anything',     color:'#3B82F6', bg:'#EFF6FF' },
          ].map(item=>(
            <button key={item.id} onClick={()=>setScreen(item.id)} style={{ borderRadius:12, border:`1px solid ${item.color}22`, background:item.bg, padding:'14px 8px', cursor:'pointer', textAlign:'center', fontFamily:'Nunito,sans-serif', transition:'all .15s' }}>
              <div style={{ fontSize:24, marginBottom:6 }}>{item.icon}</div>
              <div style={{ fontSize:12, fontWeight:800, color:'#1E1033' }}>{item.label}</div>
              <div style={{ fontSize:10, color:item.color, fontWeight:700, marginTop:3 }}>{item.sub}</div>
            </button>
          ))}
        </div>
      </Card>
