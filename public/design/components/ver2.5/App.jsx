...
function App() {
  const [onboarded, setOnboarded] = React.useState(
    () => !!localStorage.getItem('uma_onboarded')
  );
  ...
  const navigate = (s) => { setScreen(s); localStorage.setItem('uma_screen', s); };
  ...
  const renderScreen = () => {
    switch(screen) {
      ...
      case 'reports':          return <CareerReports />;
      case 'race-targets':     return <TargetRacePlanning />;
      case 'skill-loadout':    return <SkillLoadout />;
      case 'card-collection':  return <SupportCardCollection />;
      case 'mcp':           return <MCPMonitor />;
      case 'performance':   return <Performance />;
      case 'external-sync': return <ExternalSync />;
