<div class="space-y-6">
    ...
    <div class="flex flex-wrap gap-4">
        <div class="flex-1 min-w-[200px]">
            <label for="external-rarity" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Rarity
            </label>
            <select id="external-rarity" x-model="externalFilters.rarity"
                class="form-select block w-full rounded-md border-gray-300 focus:border-green-500 focus:ring-green-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
        ...
        <div class="flex-1 min-w-[200px]">
            <label for="external-import-status" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Import Status
            </label>
                ...
                <div class="aspect-3/4 bg-gray-100 dark:bg-gray-700 relative">
                    ...
                    <div
                        ...
                        <button @click="importCard(card)" :disabled="isImported(card.id)"
                            ...
                            :class="isImported(card.id) ? 'bg-gray-600 text-gray-400 cursor-not-allowed' :
    ...
    <div x-show="!externalLoading && !externalError && filteredExternalCards().length === 0 && externalCards.length > 0"
        ...
        <p class="text-sm font-medium text-gray-900 dark:text-white mb-1">No cards match your filters</p>
        <p class="text-xs text-gray-500 dark:text-gray-400">Try adjusting your filter settings</p>
    </div>
    ...
    <div x-show="!externalLoading && !externalError && externalCards.length === 0" class="text-center py-12">
        ...
        <p class="text-sm font-medium text-gray-900 dark:text-white mb-1">No external cards loaded</p>
        <p class="text-xs text-gray-500 dark:text-gray-400">Click the refresh button to load cards from the API</p>
    </div>
</div>
<?php /**PATH C:\XAMPP\htdocs\umamusume-career-planner\resources\views/support-cards/partials/external-import.blade.php ENDPATH**/ ?>