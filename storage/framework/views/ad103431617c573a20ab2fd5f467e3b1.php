...
<?php $component = App\View\Components\Breadcrumb::resolve(['items' => [['label' => 'Profile']]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
    ...
    <div class="space-y-6" x-data="profileManager()">
        ...
        <form action="<?php echo e(route('profile.update')); ?>" method="POST" class="space-y-6">
            ...
            <div x-show="activeTab === 'account'" x-transition role="tabpanel">
                <?php echo $__env->make('profile.partials.account-tab-content', ['user' => $user, 'stats' => $stats], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            </div>

            <!-- Preferences Tab -->
            <div x-show="activeTab === 'preferences'" x-transition role="tabpanel">
                <?php echo $__env->make('profile.partials.preferences-tab-content', ['user' => $user], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            </div>

            <!-- Notifications Tab -->
            <div x-show="activeTab === 'notifications'" x-transition role="tabpanel">
                <?php echo $__env->make('profile.partials.notifications-tab-content', ['user' => $user], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            </div>

            <!-- Privacy & Data Tab -->
            <div x-show="activeTab === 'privacy'" x-transition role="tabpanel">
                <?php echo $__env->make('profile.partials.privacy-tab-content', ['user' => $user], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            </div>

            <!-- Security Tab (Separate Forms) -->
            <div x-show="activeTab === 'security'" x-transition role="tabpanel">
                <?php echo $__env->make('profile.partials.security-tab', ['user' => $user], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            </div>
            ...
            window.avatarUploader = function avatarUploader() {
                return {
                    ...
                    async uploadAvatar(file) {
                        ...
                        try {
                            const response = await fetch('<?php echo e(route('profile.avatar')); ?>', {
                                method: 'POST',
                                headers: {
                    ...
                    async removeAvatar() {
                        ...
                        try {
                            const response = await fetch('<?php echo e(route('profile.avatar.delete')); ?>', {
                                method: 'DELETE',
                                headers: {
                                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                                    'Accept': 'application/json',
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\XAMPP\htdocs\umamusume-career-planner\resources\views/profile/show.blade.php ENDPATH**/ ?>