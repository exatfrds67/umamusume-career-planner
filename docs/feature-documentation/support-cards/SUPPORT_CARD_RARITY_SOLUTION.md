...
The umapyoi.net API `/api/v1/support` endpoint does not provide rarity information (R/SR/SSR) for
support cards. All cards were being defaulted to "R" rarity, making it impossible to filter or sort
by rarity accurately.

## Solution

Implemented **rarity inference based on card ID ranges**, which follows the Uma Musume game's
internal card ID structure.
