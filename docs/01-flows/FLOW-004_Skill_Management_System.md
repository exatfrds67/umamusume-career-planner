...
This flow details how `SkillService` manages the purchase of skills, including validation of SP
costs, prerequisites, and applying hint discounts.

Local mode updates skill and hint state inside the active run payload. Account mode persists the
same changes to database-backed acquisition and hint records.
...
flowchart TD
    ...
    ConfirmPurchase --> DeductSP[Deduct SP from Active Career State]
    DeductSP --> AddRecord[Persist Skill Acquisition State]

    AddRecord --> StorageMode{Storage Mode?}
    StorageMode -->|Local| SaveLocalSkill[Update Local Skill State in Run Payload]
    StorageMode -->|Account| AccountAuth{Authorized and valid?}

    AccountAuth -->|No| RejectAccount[Reject Purchase and Return Error]
    AccountAuth -->|Yes| SaveAccountSkill[Create or Update Database-Backed Skill Acquisition Record]

    SaveLocalSkill --> UpdateUI[Update Shop UI]
    SaveAccountSkill --> SaveOk{Saved?}
...
Hint persistence should branch by storage mode instead of assuming a database write for every update.
...
flowchart TD
    ...
    IncrementLevel --> UpdateHintState[Persist Hint Level State]
    UpdateHintState --> StorageMode{Storage Mode?}
    StorageMode -->|Local| SaveLocalHint[Update Local Hint State in Run Payload]
    StorageMode -->|Account| SaveAccountHint[Update Database-Backed Hint Record]

    SaveLocalHint --> CalcDiscount[Recalculate Discount %]
    SaveAccountHint --> CalcDiscount
...
| **Fast Learner** | Temporary character condition | +10% additional discount (stacks with hint level); granted by specific support card events and not permanently active |
...
flowchart TD
    ...
    ConvertToWeak --> PersistHint{Storage Mode?}
    KeepStandard --> PersistHint

    PersistHint -->|Local| StoreHintLocal[Store in Local Run Payload]
    PersistHint -->|Account| StoreHintAccount[Store in Database-Backed Hint Records]
```
