...
After comprehensive analysis of the codebase against verified game mechanics from authoritative
sources, I've identified **4 critical discrepancies** and **12 high-priority alignment tasks** that
require immediate attention. The current implementation contains fundamental calculation errors that
affect core gameplay simulation accuracy.
...
**Rationale**: Skill hint system affects every SP calculation in the application. This is the
highest-impact correction. Aptitude fix is quick and prevents future data corruption.
...
invokeSubAgent(
  ...
  prompt: "Create spec for correcting skill hint discount system from 20%/40% (max 2 hints) to
  10%/20%/30%/35%/40% (max 5 hints) based on verified game mechanics",
