...
- **Parameter**: `failure_threshold`; **Value**: 5; **Description**: Number of failures to open circuit
- **Parameter**: `recovery_timeout`; **Value**: 60 seconds; **Description**: Time before attempting recovery
- **Parameter**: `sample_window`; **Value**: 120 seconds; **Description**: Time window for failure counting
- **Parameter**: `half_open_requests`; **Value**: 3; **Description**: Number of probe requests in half-open state
...
- **Algorithm**: Goal Priority Weighting; **Purpose**: Align training to character goals; **Weight**: 35%
- **Algorithm**: Turn Economy Analysis; **Purpose**: Optimize turn usage vs remaining turns; **Weight**: 25%
- **Algorithm**: Risk-Reward Calculation; **Purpose**: Balance failure risk vs potential gains; **Weight**: 20%
- **Algorithm**: Long-term Impact; **Purpose**: Project career trajectory; **Weight**: 20%
...
- **Provider**: Ollama; **Model**: llama3.2; **Input Cost**: $0.00; **Output Cost**: $0.00; **Use Case**: Local, privacy-first
- **Provider**: Bedrock; **Model**: Claude Haiku; **Input Cost**: $1.00/1M; **Output Cost**: $5.00/1M; **Use Case**: Quick responses
- **Provider**: Bedrock; **Model**: Claude Sonnet; **Input Cost**: $3.00/1M; **Output Cost**: $15.00/1M; **Use Case**: Strategic advice
- **Provider**: Bedrock; **Model**: Claude Opus; **Input Cost**: $5.00/1M; **Output Cost**: $25.00/1M; **Use Case**: Complex reasoning
- **Provider**: Bedrock; **Model**: Nova Lite; **Input Cost**: $0.00125/1K; **Output Cost**: $0.00125/1K; **Use Case**: Budget-friendly
- **Provider**: Bedrock; **Model**: Nova Pro; **Input Cost**: Preview; **Output Cost**: Preview; **Use Case**: Advanced capabilities
...
- **Server**: `memory`; **Type**: Local; **Purpose**: Conversation context; **Enabled**: Yes
- **Server**: `filesystem`; **Type**: Local; **Purpose**: File operations; **Enabled**: Yes
- **Server**: `fetch`; **Type**: Local; **Purpose**: HTTP requests; **Enabled**: Yes
- **Server**: `umapyoi`; **Type**: Remote; **Purpose**: Game data API (optional); **Enabled**: No (default)
...
- **Metric Category**: **Training Efficiency**; **Metrics Tracked**: Stat gains per turn, training success rate, optimal facility usage
- **Metric Category**: **Prediction Accuracy**; **Metrics Tracked**: Expected vs actual stat gains, risk prediction accuracy
- **Metric Category**: **Goal Progress**; **Metrics Tracked**: Goal completion trajectory, turn-based progress rate
- **Metric Category**: **Resource Utilization**; **Metrics Tracked**: SP efficiency, energy management, item usage optimization
- **Metric Category**: **Pattern Recognition**; **Metrics Tracked**: Successful decision patterns, failure point identification, optimal timing windows
- **Metric Category**: **Deck Performance**; **Metrics Tracked**: Support card synergy scores, friendship training frequency, bond progression
...
- **Operation**: **Resize**; **Library**: GD; **Purpose**: Normalize dimensions; **Configuration**: Max 2000px width
- **Operation**: **Grayscale**; **Library**: GD; **Purpose**: Improve contrast; **Configuration**: `imagefilter(IMG_FILTER_GRAYSCALE)`
- **Operation**: **Threshold**; **Library**: GD; **Purpose**: Binary conversion; **Configuration**: Adaptive threshold
- **Operation**: **Denoise**; **Library**: GD; **Purpose**: Remove artifacts; **Configuration**: Median filter

### 7.4 Supported Data Types

- **Data Type**: **Character Stats**; **Detection Pattern**: Stat labels + numeric values (0-1200); **Confidence Threshold**: 85%
...
- **Error Category**: **API Connection**; **Detection Method**: Timeout, network error; **Recovery Strategy**: Exponential backoff retry, cache fallback; **Escalation Path**: Circuit breaker open after 5 failures
- **Error Category**: **Database**; **Detection Method**: SQL exception, constraint violation; **Recovery Strategy**: Transaction rollback, connection reset; **Escalation Path**: DBA notification after repeated failures
- **Error Category**: **AI Model**; **Detection Method**: Provider timeout, rate limit; **Recovery Strategy**: Fallback to alternative provider; **Escalation Path**: Cost alert if fallback usage exceeds threshold
- **Error Category**: **OCR Processing**; **Detection Method**: Low confidence score; **Recovery Strategy**: Manual correction UI; **Escalation Path**: User assistance request
- **Error Category**: **Calculation**; **Detection Method**: Invalid input, out of range; **Recovery Strategy**: Input sanitization, safe defaults; **Escalation Path**: Log for algorithm review
...
### Broadcast Events
- **Event**: `TrainingCompleted`; **Channel**: `character.{id}`; **Data**: Updated stats, energy, mood, skill hints
- **Event**: `RaceCompleted`; **Channel**: `character.{id}`; **Data**: Race results, fan gains, SP gains
- **Event**: `SkillAcquired`; **Channel**: `character.{id}`; **Data**: Skill details, SP cost, remaining SP
- **Event**: `AIRecommendation`; **Channel**: `user.{id}`; **Data**: Recommendation content, confidence score
- **Event**: `ExternalDataUpdated`; **Channel**: `public`; **Data**: Updated meta tier, community data
...
- **Process**: **External Data Sync**; **Technology**: HTTP Client + Circuit Breaker; **Primary Purpose**: Game data synchronization; **Performance Target**: Response < 2s, Fallback < 500ms
- **Process**: **Training Prediction**; **Technology**: Service Layer + Caching; **Primary Purpose**: Stat gain calculations; **Performance Target**: Response < 200ms (cached), < 1.2s (calculated)
- **Process**: **AI Advisory**; **Technology**: Ollama + AWS Bedrock; **Primary Purpose**: Intelligent recommendations; **Performance Target**: Ollama < 5s, Bedrock < 10s
- **Process**: **MCP Tool Execution**; **Technology**: MCP Client + Servers; **Primary Purpose**: Agent tool access; **Performance Target**: Response < 2s per tool
- **Process**: **Career Analytics**; **Technology**: Database + Background Jobs; **Primary Purpose**: Performance tracking; **Performance Target**: Real-time metrics, Async reports
- **Process**: **OCR Processing**; **Technology**: Tesseract + GD; **Primary Purpose**: Screenshot parsing; **Performance Target**: Processing < 10s per image
- **Process**: **Error Recovery**; **Technology**: Circuit Breaker + Fallbacks; **Primary Purpose**: System resilience; **Performance Target**: Automatic recovery within 60s
- **Process**: **WebSocket Broadcast**; **Technology**: Laravel Reverb; **Primary Purpose**: Real-time updates; **Performance Target**: Push delay < 100ms
...
- **Metric**: **Page Load Time**; **Target**: < 2 seconds; **Current**: ~2.2s; **Status**: 🟡 Optimizing
- **Metric**: **First Contentful Paint**; **Target**: < 1.5 seconds; **Current**: ~1.7s; **Status**: 🟡 Optimizing
- **Metric**: **Time to Interactive**; **Target**: < 3 seconds; **Current**: ~3.1s; **Status**: 🟡 Optimizing
- **Metric**: **API Response Time (p95)**; **Target**: < 500ms; **Current**: ~420ms; **Status**: ✅ Met
- **Metric**: **Cache Hit Rate**; **Target**: > 85%; **Current**: 88%; **Status**: ✅ Met
- **Metric**: **AI Response Time (Ollama)**; **Target**: < 5 seconds; **Current**: ~4.2s; **Status**: ✅ Met
- **Metric**: **AI Response Time (Bedrock)**; **Target**: < 10 seconds; **Current**: ~8.5s; **Status**: ✅ Met
- **Metric**: **WebSocket Latency**; **Target**: < 100ms; **Current**: ~75ms; **Status**: ✅ Met
- **Metric**: **OCR Processing**; **Target**: < 10 seconds; **Current**: ~8.3s; **Status**: ✅ Met

### 10.4 Scalability Considerations

- **Component**: **Web Tier**; **Scaling Strategy**: Horizontal (Load Balancer); **Current Limit**: 10 req/s per instance; **Notes**: Stateless design enables easy scaling
- **Component**: **Database**; **Scaling Strategy**: Read replicas + Partitioning; **Current Limit**: 100 concurrent connections; **Notes**: Indexed for query optimization
- **Component**: **Cache (Redis)**; **Scaling Strategy**: Redis Cluster; **Current Limit**: 10,000 operations/s; **Notes**: In-memory for fast access
- **Component**: **AI Processing**; **Scaling Strategy**: Provider-based (unlimited Bedrock); **Current Limit**: Ollama: 1 concurrent; **Notes**: Bedrock scales automatically
- **Component**: **WebSocket**; **Scaling Strategy**: Reverb Horizontal Scaling; **Current Limit**: 1,000 concurrent connections; **Notes**: Redis pub/sub for multi-server
- **Component**: **OCR Pipeline**; **Scaling Strategy**: Queue-based processing; **Current Limit**: 10 concurrent jobs; **Notes**: Background job processing
...
- **Service**: `ApmService`; **Purpose**: Core APM metrics collection and reporting
- **Service**: `ApiPerformanceMonitoringService`; **Purpose**: API endpoint performance tracking
- **Service**: `QueryOptimizationService`; **Purpose**: Database query analysis and optimization
- **Service**: `PerformanceAlertingService`; **Purpose**: Alert generation and threshold management
- **Service**: `RedisCacheOptimizationService`; **Purpose**: Cache hit/miss analysis and optimization
- **Service**: `ApiResponseCachingService`; **Purpose**: Response caching strategies and invalidation
- **Service**: `PerformanceRegressionService`; **Purpose**: Regression detection across deployments
- **Service**: `HistoricalTrackingService`; **Purpose**: Long-term metrics storage and analysis
...
- **Version**: 2.4.0; **Date**: 2026-02-22; **Author**: Development Team; **Changes**: Fixed TypeScript→Livewire 4/Alpine.js 3 references; added Neuron AI v2.11; updated external API references (GameTora); added Admin Panel to architecture; updated stats
- **Version**: 2.3.0; **Date**: 2026-02-21; **Author**: Development Team; **Changes**: Updated version/date metadata; Livewire 3→4; aligned with 30 current Eloquent models
- **Version**: 2.2.0; **Date**: 2026-01-27; **Author**: Development Team; **Changes**: Added §11 Performance Monitoring & APM Flow; added 8 Performance & Monitoring services
- **Version**: 2.1.0; **Date**: 2026-01-23; **Author**: Development Team; **Changes**: Comprehensive update for v2.0.0: Added MCP integration, OCR pipeline, real-time communication flows; aligned with current implementation
- **Version**: 2.0.0; **Date**: 2026-01-14; **Author**: Development Team; **Changes**: Major revision with Mermaid diagrams
