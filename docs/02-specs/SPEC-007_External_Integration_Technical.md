...
- **Attribute**: **Document ID**; **Value**: SPEC-007
- **Attribute**: **Related PRD**; **Value**: [PRD-007: External Integration](../prds/PRD-007_External_Integration.md)
...
- **Component**: **HTTP Client**; **Technology**: Guzzle/Laravel HTTP; **Version**: Latest; **Purpose**: API requests
- **Component**: **OCR Engine**; **Technology**: Tesseract; **Version**: 5.x; **Purpose**: Text extraction
- **Component**: **Image Processing**; **Technology**: GD Library; **Version**: 2.x; **Purpose**: Image preprocessing
- **Component**: **WebSocket**; **Technology**: Laravel Reverb; **Version**: 1.x; **Purpose**: Real-time updates
- **Component**: **Cache**; **Technology**: Redis; **Version**: 7.x; **Purpose**: Response caching
- **Component**: **Queue**; **Technology**: Laravel Queue; **Version**: 12.x; **Purpose**: Background jobs
...
- **Pattern**: **Circuit Breaker**; **Implementation**: `CircuitBreaker` class; **Purpose**: Prevent cascade failures
- **Pattern**: **Adapter**; **Implementation**: API client classes; **Purpose**: Abstract external APIs
- **Pattern**: **Strategy**; **Implementation**: Parser implementations; **Purpose**: Pluggable parsing logic
- **Pattern**: **Observer**; **Implementation**: Event listeners; **Purpose**: React to sync events
- **Pattern**: **Cache-Aside**; **Implementation**: Response caching; **Purpose**: Performance optimization
- **Pattern**: **Retry**; **Implementation**: HTTP client config; **Purpose**: Transient failure handling
- **Pattern**: **Factory**; **Implementation**: Client factory; **Purpose**: API client instantiation
...
- **Method**: GET; **Endpoint**: `/api/v1/external/characters/{traineeId}`; **Description**: Fetch character data from external API; **Auth Required**: Yes
- **Method**: GET; **Endpoint**: `/api/v1/external/support-cards`; **Description**: Fetch support card database; **Auth Required**: Yes
- **Method**: GET; **Endpoint**: `/api/v1/external/skills`; **Description**: Fetch skill catalog; **Auth Required**: Yes
- **Method**: GET; **Endpoint**: `/api/v1/external/races`; **Description**: Fetch race definitions; **Auth Required**: Yes
- **Method**: GET; **Endpoint**: `/api/v1/external/health`; **Description**: Get external API health status; **Auth Required**: Yes
- **Method**: POST; **Endpoint**: `/api/v1/external/sync`; **Description**: Trigger data synchronization; **Auth Required**: Yes
- **Method**: POST; **Endpoint**: `/api/v1/ocr/process`; **Description**: Process screenshot via OCR; **Auth Required**: Yes
- **Method**: GET; **Endpoint**: `/api/v1/ocr/extractions/{id}`; **Description**: Get OCR extraction result; **Auth Required**: Yes
- **Method**: PATCH; **Endpoint**: `/api/v1/ocr/extractions/{id}/verify`; **Description**: Verify and correct OCR data; **Auth Required**: Yes
- **Method**: GET; **Endpoint**: `/api/v1/community/tips/{traineeId}`; **Description**: Get community tips for character; **Auth Required**: Yes
- **Method**: POST; **Endpoint**: `/api/v1/community/share`; **Description**: Share career results to community; **Auth Required**: Yes
- **Method**: GET; **Endpoint**: `/api/v1/community/share/{token}`; **Description**: Get shared career data; **Auth Required**: No
...
- **Layer**: **Response Cache**; **Technology**: Redis; **TTL**: 24 hours; **Purpose**: External API responses
- **Layer**: **Stale Cache**; **Technology**: Redis; **TTL**: 7 days; **Purpose**: Fallback when APIs unavailable
- **Layer**: **OCR Results**; **Technology**: Database; **TTL**: Permanent; **Purpose**: Extraction history
- **Layer**: **Circuit State**; **Technology**: Redis; **TTL**: 24 hours; **Purpose**: Circuit breaker state
- **Layer**: **Meta Rankings**; **Technology**: Redis; **TTL**: 12 hours; **Purpose**: Community tier lists
...
- **Code**: `EXT_API_TIMEOUT`; **HTTP Status**: 504; **Description**: External API request timeout; **Resolution**: Retry or use cached data
- **Code**: `EXT_API_UNAVAILABLE`; **HTTP Status**: 503; **Description**: All external APIs unavailable; **Resolution**: Use stale cached data
- **Code**: `EXT_RATE_LIMITED`; **HTTP Status**: 429; **Description**: Rate limit exceeded; **Resolution**: Wait and retry
- **Code**: `CIRCUIT_OPEN`; **HTTP Status**: 503; **Description**: Circuit breaker is open; **Resolution**: Use cached data
- **Code**: `OCR_INVALID_IMAGE`; **HTTP Status**: 422; **Description**: Invalid image format; **Resolution**: Upload valid image
- **Code**: `OCR_ENGINE_ERROR`; **HTTP Status**: 500; **Description**: Tesseract processing failed; **Resolution**: Retry or manual entry
- **Code**: `OCR_PARSE_FAILED`; **HTTP Status**: 422; **Description**: Could not parse OCR text; **Resolution**: Manual data entry
- **Code**: `OCR_LOW_CONFIDENCE`; **HTTP Status**: 422; **Description**: Confidence below threshold; **Resolution**: Manual review required
- **Code**: `SHARE_FAILED`; **HTTP Status**: 500; **Description**: Could not create share; **Resolution**: Retry later
...
- **Security Measure**: **Authentication**; **Implementation**: All endpoints require Sanctum token
- **Security Measure**: **Rate Limiting**; **Implementation**: 100 requests/minute per user
- **Security Measure**: **Input Validation**; **Implementation**: Form Request validation on all inputs
- **Security Measure**: **Output Sanitization**; **Implementation**: JSON encoding with proper escaping
- **Security Measure**: **CORS**; **Implementation**: Configured for allowed origins only

### 13.2 External API Security

- **Security Measure**: **API Keys**; **Implementation**: Stored in environment variables, never in code
- **Security Measure**: **HTTPS Only**; **Implementation**: All external API calls use HTTPS
- **Security Measure**: **Response Validation**; **Implementation**: JSON schema validation on responses
- **Security Measure**: **Data Sanitization**; **Implementation**: All external data sanitized before storage

### 13.3 OCR Security

- **Security Measure**: **File Validation**; **Implementation**: MIME type sniffing, extension validation
- **Security Measure**: **Size Limits**; **Implementation**: Maximum 10MB file size
- **Security Measure**: **Temporary Storage**; **Implementation**: Processed images deleted after extraction
- **Security Measure**: **Content Scanning**; **Implementation**: Validate image contains expected content
...
- **Operation**: External API call (cached); **Target**: < 50ms; **Measurement**: p95
- **Operation**: External API call (fresh); **Target**: < 2s; **Measurement**: p95
- **Operation**: OCR processing; **Target**: < 5s; **Measurement**: p95
- **Operation**: Circuit breaker check; **Target**: < 5ms; **Measurement**: p95
- **Operation**: WebSocket broadcast; **Target**: < 100ms; **Measurement**: p95
- **Operation**: Community share creation; **Target**: < 500ms; **Measurement**: p95
...
- **Endpoint**: `/api/v1/characters`; **Method**: GET; **Description**: List all characters; **Rate Limit**: 60/min
- **Endpoint**: `/api/v1/characters/{id}`; **Method**: GET; **Description**: Get character by ID; **Rate Limit**: 60/min
- **Endpoint**: `/api/v1/support-cards`; **Method**: GET; **Description**: List support cards; **Rate Limit**: 60/min
- **Endpoint**: `/api/v1/skills`; **Method**: GET; **Description**: List skills; **Rate Limit**: 60/min
- **Endpoint**: `/api/v1/races`; **Method**: GET; **Description**: List races; **Rate Limit**: 60/min
- **Endpoint**: `/api/v1/meta/tiers`; **Method**: GET; **Description**: Get meta tier rankings; **Rate Limit**: 30/min
- **Endpoint**: `/api/v1/health`; **Method**: GET; **Description**: Health check; **Rate Limit**: 120/min

**GameTora API**:

- **Endpoint**: `/api/characters`; **Method**: GET; **Description**: List all characters; **Rate Limit**: 30/min
- **Endpoint**: `/api/characters/{id}`; **Method**: GET; **Description**: Get character by ID; **Rate Limit**: 30/min
- **Endpoint**: `/api/support-cards`; **Method**: GET; **Description**: List support cards; **Rate Limit**: 30/min
- **Endpoint**: `/api/skills`; **Method**: GET; **Description**: List skills; **Rate Limit**: 30/min
- **Endpoint**: `/api/status`; **Method**: GET; **Description**: Health check; **Rate Limit**: 60/min

### Appendix B: OCR Pattern Reference

**Stat Patterns**:

- **Pattern**: `/Speed[:\s]*(\d{1,4})/i`; **Language**: English; **Example Match**: "Speed: 850"
- **Pattern**: `/スピード[:\s]*(\d{1,4})/`; **Language**: Japanese; **Example Match**: "スピード: 850"
- **Pattern**: `/Stamina[:\s]*(\d{1,4})/i`; **Language**: English; **Example Match**: "Stamina: 720"
- **Pattern**: `/スタミナ[:\s]*(\d{1,4})/`; **Language**: Japanese; **Example Match**: "スタミナ: 720"
- **Pattern**: `/Power[:\s]*(\d{1,4})/i`; **Language**: English; **Example Match**: "Power: 680"
- **Pattern**: `/パワー[:\s]*(\d{1,4})/`; **Language**: Japanese; **Example Match**: "パワー: 680"
- **Pattern**: `/Guts[:\s]*(\d{1,4})/i`; **Language**: English; **Example Match**: "Guts: 550"
- **Pattern**: `/根性[:\s]*(\d{1,4})/`; **Language**: Japanese; **Example Match**: "根性: 550"
- **Pattern**: `/(?:Wit\; **Language**: Wisdom)[:\s]*(\d{1,4})/i`; **Example Match**: English; **Col4**: "Wit: 620"
- **Pattern**: `/賢さ[:\s]*(\d{1,4})/`; **Language**: Japanese; **Example Match**: "賢さ: 620"

**Mood Patterns**:

- **Pattern**: `Great`, `絶好調`; **Normalized Value**: `great`
- **Pattern**: `Good`, `好調`; **Normalized Value**: `good`
- **Pattern**: `Normal`, `普通`; **Normalized Value**: `normal`
- **Pattern**: `Bad`, `不調`; **Normalized Value**: `bad`
- **Pattern**: `Awful`, `絶不調`; **Normalized Value**: `awful`

**Aptitude Patterns**:

- **Pattern**: `/([A-G]\; **Valid Values**: S{1,2})/i`; **Col3**: SS, S, A, B, C, D, E, F, G

### Appendix C: Circuit Breaker Configuration

**Default Settings**:

- **Parameter**: `failure_threshold`; **Value**: 5; **Description**: Failures before opening circuit
- **Parameter**: `recovery_timeout`; **Value**: 60 seconds; **Description**: Time before testing recovery
- **Parameter**: `sample_window`; **Value**: 120 seconds; **Description**: Window for counting failures
- **Parameter**: `half_open_requests`; **Value**: 1; **Description**: Probe requests in half-open state

**State Transitions**:

- **From State**: Closed; **Condition**: Failures >= threshold; **To State**: Open
- **From State**: Open; **Condition**: Recovery timeout elapsed; **To State**: Half-Open
- **From State**: Half-Open; **Condition**: Probe succeeds; **To State**: Closed
- **From State**: Half-Open; **Condition**: Probe fails; **To State**: Open

### Appendix D: WebSocket Channel Reference

**Private Channels**:

- **Channel Pattern**: `character.{id}`; **Authorization**: Owner only; **Events**: `character.updated`, `training.completed`
- **Channel Pattern**: `user.{id}`; **Authorization**: User only; **Events**: `notification`, `sync.complete`

**Public Channels**:

- **Channel**: `global.updates`; **Events**: `data.synced`, `maintenance.scheduled`
...
- **Code**: `EXT_API_TIMEOUT`; **HTTP Status**: 504; **Category**: External API; **Description**: Request timed out
- **Code**: `EXT_API_UNAVAILABLE`; **HTTP Status**: 503; **Category**: External API; **Description**: All APIs unavailable
- **Code**: `EXT_RATE_LIMITED`; **HTTP Status**: 429; **Category**: External API; **Description**: Rate limit exceeded
- **Code**: `CIRCUIT_OPEN`; **HTTP Status**: 503; **Category**: Circuit Breaker; **Description**: Circuit is open
- **Code**: `CIRCUIT_HALF_OPEN`; **HTTP Status**: 503; **Category**: Circuit Breaker; **Description**: Limited requests allowed
- **Code**: `OCR_INVALID_IMAGE`; **HTTP Status**: 422; **Category**: OCR; **Description**: Invalid image format
- **Code**: `OCR_ENGINE_ERROR`; **HTTP Status**: 500; **Category**: OCR; **Description**: Tesseract processing failed
- **Code**: `OCR_PARSE_FAILED`; **HTTP Status**: 422; **Category**: OCR; **Description**: Could not parse extracted text
- **Code**: `OCR_LOW_CONFIDENCE`; **HTTP Status**: 422; **Category**: OCR; **Description**: Confidence below threshold
- **Code**: `SHARE_FAILED`; **HTTP Status**: 500; **Category**: Community; **Description**: Share creation failed
- **Code**: `SHARE_NOT_FOUND`; **HTTP Status**: 404; **Category**: Community; **Description**: Share token invalid
- **Code**: `SHARE_EXPIRED`; **HTTP Status**: 410; **Category**: Community; **Description**: Share has expired
...
- **Version**: 2.3.0; **Date**: 2026-02-22; **Author**: Development Team; **Changes**: Updated to v2.3.0 architecture: GameTora replaces UmamusumeDB, ExternalDataService replaces ExternalAPIService, GameToraScraperService replaces UmamusumeDBApiClient, PHP 8.2+, status complete
- **Version**: 2.2.0; **Date**: 2026-01-28; **Author**: Development Team; **Changes**: Updated to align with game-accurate mechanics (v2.2.0 architecture), data validation for aptitude grades (G-S)
- **Version**: 2.0.0; **Date**: 2026-01-24; **Author**: Development Team; **Changes**: Full v2.0.0 alignment, complete technical specification with all services, API endpoints, database schema, testing strategy, and comprehensive appendices
...
- **Role**: DevOps Lead; **Name**: [Name]; **Signature**: _________; **Date**: 2026-01-24
