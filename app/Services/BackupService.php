...
class BackupService
{
    ...
    public function restoreFromBackup(string $backupId, int $userId, array $options = []): array
    {
        ...
        try {
            ...
            $filePath = is_string($backupRecord['file_path'] ?? null) ? (string) $backupRecord['file_path'] : '';
            $encrypted = (bool) ($backupRecord['encrypted'] ?? false);
            $checksum = is_string($backupRecord['checksum'] ?? null) ? (string) $backupRecord['checksum'] : '';
            ...
            $typedData = $data;
            $integrityCheck = $this->verifyDataIntegrity($typedData, $checksum, $filePath);
    ...
    private function restoreCharacters(array $characters, int $userId, bool $overwrite): array
    {
        ...
        foreach ($characters as $charData) {
            ...
            try {
                $charName = is_string($charData['name'] ?? null) ? (string) $charData['name'] : 'Unknown';
    ...
    private function restoreCareers(array $careers, int $userId, bool $overwrite): array
    {
        ...
        foreach ($careers as $careerData) {
            ...
            try {
                $careerName = is_string($careerData['career_name'] ?? null) ? (string) $careerData['career_name'] : 'Unknown';
    ...
    private function restoreSkills(array $skills, bool $overwrite): array
    {
        ...
        foreach ($skills as $skillData) {
            ...
            try {
                $skillName = is_string($skillData['name'] ?? null) ? (string) $skillData['name'] : 'Unknown';
    ...
    private function restoreSupportCards(array $cards, bool $overwrite): array
    {
        ...
        foreach ($cards as $cardData) {
            ...
            try {
                $cardName = is_string($cardData['name'] ?? null) ? (string) $cardData['name'] : 'Unknown';
    ...
    public function scheduleBackup(int $userId, array $scheduleConfig = []): array
    {
        ...
        $frequency = is_string($scheduleConfig['frequency'] ?? null) ? (string) $scheduleConfig['frequency'] : self::SCHEDULE_DAILY;
        $time = is_string($scheduleConfig['time'] ?? null) ? (string) $scheduleConfig['time'] : '02:00';
    ...
    public function updateBackupSchedule(string $scheduleId, int $userId, array $updates): array
    {
        ...
        $frequency = is_string($schedule['frequency'] ?? null) ? (string) $schedule['frequency'] : self::SCHEDULE_DAILY;
        $time = is_string($schedule['time'] ?? null) ? (string) $schedule['time'] : '02:00';
    ...
    public function listBackups(int $userId, array $filters = []): array
    {
        try {
            ...
            usort($backups, function ($a, $b) {
                $aTime = is_string($a['created_at'] ?? null) ? (string) $a['created_at'] : '';
                $bTime = is_string($b['created_at'] ?? null) ? (string) $b['created_at'] : '';

                return strcmp($bTime, $aTime);
            });

            // Apply pagination
            $page = is_int($filters['page'] ?? null) ? (int) $filters['page'] : 1;
            $perPage = is_int($filters['per_page'] ?? null) ? (int) $filters['per_page'] : 20;
            $offset = ($page - 1) * $perPage;
            $total = count($backups);
            $backups = array_slice($backups, (int) $offset, (int) $perPage);
    ...
    public function deleteBackup(string $backupId, int $userId): array
    {
        try {
            ...
            // Delete file
            $filePath = is_string($backup['file_path'] ?? null) ? (string) $backup['file_path'] : '';
