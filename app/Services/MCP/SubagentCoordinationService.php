...
class SubagentCoordinationService
{
    ...
    protected function executeAgent(string $agent, array $context): array
    {
        ...
        $response = $this->aiService->processRequest($prompt, [
            'agent' => $agent,
            'context' => $context,
        ]);
     ...
     *
     * @param  array<string, array<string, mixed>>  $results  Individual agent results
     * @return array<string, mixed> Aggregated results
     */
    protected function aggregateResults(array $results): array
    {
        $successful = array_filter($results, fn (array $r): bool => ($r['status'] ?? '') === 'success');
        $failed = array_filter($results, fn (array $r): bool => ($r['status'] ?? '') === 'failed');
