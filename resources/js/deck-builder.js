export default (initialDeck = [], characterId = null) => ({
    ...
    get filteredCards() {
        let cards = this.availableCards.filter((card) => {
            ...
            const isInDeck = this.currentDeck.some(
    ...
    async addCard(cardId, isFriend = false) {
        if (!this.characterId) {
        ...
        try {
            // Make API call to persist the card
            const response = await axios.post(
                `/api/v1/characters/${this.characterId}/deck/add-card`,
                {
    ...
    // Alias for addCard to match the view's selectCard calls
    async selectCard(cardId, isFriend = false) {
        return this.addCard(cardId, isFriend);
    },
    ...
    async removeCard(slot) {
        ...
        try {
            const response = await axios.delete(
                `/api/v1/characters/${this.characterId}/deck/remove-card/${slot}`,
            );
    ...
    handleDrop(event, targetSlot) {
        ...
        const sourceCardIndex = this.currentDeck.findIndex(
            (c) => c.position_slot === sourceSlot,
        );
        const targetCardIndex = this.currentDeck.findIndex(
    ...
    swapSlots(slotA, slotB) {
        const indexA = this.currentDeck.findIndex(
            (c) => c.position_slot === slotA,
        );
        const indexB = this.currentDeck.findIndex(
    ...
    validateDeck() {
        ...
        if (this.deckCount !== 6) {
            this.validationErrors.push(
                `Deck must have exactly 6 cards (currently ${this.deckCount}).`,
            );
        }
        ...
        if (this.friendCardCount === 0 && this.deckCount > 0) {
            this.validationWarnings.push(
                "Consider adding a friend card for slot 6.",
            );
        }
        ...
        const duplicates = cardIds.filter(
            (id, idx) => cardIds.indexOf(id) !== idx,
        );
        if (duplicates.length > 0) {
            this.validationErrors.push(
                "Duplicate cards detected in owned slots.",
            );
        }
        ...
        // Meta tier warning
        const cards = this.currentDeck.filter((c) => c && c.supportCard);
        const lowTierCount = cards.filter((c) =>
            ["C", "D"].includes(c.supportCard?.meta_tier),
        ).length;
        if (lowTierCount >= 3) {
            this.validationWarnings.push(
                "Multiple low-tier cards may impact performance.",
            );
        }

        // Bond level warning
        const lowBondCount = cards.filter(
    ...
    saveEditModal() {
        ...
        const cardIndex = this.currentDeck.findIndex(
            (c) => c.position_slot === this.editingSlot,
        );
        if (cardIndex > -1) {
            this.currentDeck[cardIndex].limit_break_level = Math.max(
                0,
                Math.min(4, parseInt(this.editForm.limitBreak) || 0),
            );
            this.currentDeck[cardIndex].friendship_level = Math.max(
                0,
                Math.min(100, parseInt(this.editForm.bondLevel) || 0),
            );
