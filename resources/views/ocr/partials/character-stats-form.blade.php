        ...
        @foreach (['speed' => 'Speed', 'stamina' => 'Stamina', 'power' => 'Power', 'guts' => 'Guts', 'wit' => 'Wit'] as $stat => $label)
            <div>
                ...
                <input type="number" id="stat_{{ $stat }}" name="stats[{{ $stat }}]"
                    ...
                    class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:text-white dark:border-gray-600 {{ isset($data['stats'][$stat]) && $data['stats'][$stat] === null ? 'border-yellow-500 bg-yellow-50 dark:bg-yellow-900/20' : 'border-gray-300' }}"
