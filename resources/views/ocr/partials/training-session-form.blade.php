...
<div class="space-y-6">
    <!-- Training Type -->
    <div>
        <label for="training_type" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
            Training Type
        </label>
    ...
    <!-- Stat Gains -->
    <div>
        <h3 class="text-md font-semibold text-neutral-900 dark:text-white mb-3">Stat Gains</h3>
            ...
            @foreach (['speed' => 'Speed', 'stamina' => 'Stamina', 'power' => 'Power', 'guts' => 'Guts', 'wit' => 'Wit'] as $stat => $label)
                <div>
                    ...
                    <input type="number" id="stat_gain_{{ $stat }}" name="stat_gains[{{ $stat }}]"
                        ...
                        class="w-full px-3 py-2 border border-neutral-300 dark:border-neutral-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 dark:bg-neutral-700 dark:text-white"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            <label for="energy_cost" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                Energy Cost (%)
            </label>
        ...
        <div>
            <label for="turn_number" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                Turn Number
            </label>
    ...
    <!-- Training Flags -->
    <div>
        <h3 class="text-md font-semibold text-neutral-900 dark:text-white mb-3">Training Features</h3>
        <div class="space-y-2">
            <label class="flex items-center">
                ...
                <span class="ml-2 text-sm text-neutral-700 dark:text-neutral-300">Has Skill Hint (Red !)</span>
            </label>
            <label class="flex items-center">
                <input type="checkbox" name="has_friendship" value="1"
                    {{ $data['has_friendship'] ?? false ? 'checked' : '' }}
                    class="w-4 h-4 text-primary-500 border-neutral-300 rounded focus:ring-primary-500 dark:border-neutral-600 dark:bg-neutral-700">
                <span class="ml-2 text-sm text-neutral-700 dark:text-neutral-300">Friendship Training (Rainbow)</span>
            </label>
            <label class="flex items-center">
                <input type="checkbox" name="has_spirit_burst" value="1"
                    {{ $data['has_spirit_burst'] ?? false ? 'checked' : '' }}
                    class="w-4 h-4 text-primary-500 border-neutral-300 rounded focus:ring-primary-500 dark:border-neutral-600 dark:bg-neutral-700">
                <span class="ml-2 text-sm text-neutral-700 dark:text-neutral-300">Spirit Burst Available (Unity Cup)</span>
            </label>
        </div>
    </div>
    ...
    <!-- Participants -->
    <div>
        <label for="participants" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
