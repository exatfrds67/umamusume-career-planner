<x-admin-layout title="Logs">
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <h1 class="text-3xl font-bold text-neutral-900 dark:text-white">Application Logs</h1>
        ...
        <div class="text-sm text-neutral-600 dark:text-neutral-400">Log file size: {{ $fileSize }}</div>
        ...
        <form method="GET" role="search" aria-label="Filter log entries" class="flex gap-4">
            ...
            <input type="text" name="search" id="log-search" value="{{ request('search') }}" placeholder="Search logs..."
                class="flex-1 rounded-md border-neutral-300 shadow-xs dark:border-neutral-700 dark:bg-neutral-800 dark:text-white">
            <label for="log-level-filter" class="sr-only">Log level filter</label>
            <select name="level" id="log-level-filter"
                class="rounded-md border-neutral-300 shadow-xs dark:border-neutral-700 dark:bg-neutral-800 dark:text-white">
                        ...
                        <div class="flex-1">
                            <div class="flex items-center gap-2">
                                <span @class([
                                    ...
                                    'bg-neutral-100 text-neutral-800 dark:bg-neutral-700 dark:text-neutral-200' => $log['level'] === 'DEBUG',
                                ])>
                                    {{ $log['level'] }}
                                </span>
                                <span class="text-sm text-neutral-500 dark:text-neutral-400"><time datetime="{{ $log['timestamp'] }}">{{ $log['timestamp'] }}</time></span>
                            </div>
                            <p class="mt-2 text-sm text-neutral-900 dark:text-white">{{ $log['message'] }}</p>
                            @if ($log['context'])
                                <pre aria-label="Log context" class="mt-2 overflow-x-auto rounded bg-neutral-100 p-2 text-xs text-neutral-700 dark:bg-neutral-900 dark:text-neutral-300">{{ trim($log['context']) }}</pre>
                            @endif
                        </div>
                    </div>
                </div>
            @empty
                <div class="rounded-lg bg-white p-8 text-center shadow dark:bg-neutral-800">
                    <p class="text-neutral-500 dark:text-neutral-400">No logs found.</p>
                </div>
            @endforelse
        </div>
    </div>
