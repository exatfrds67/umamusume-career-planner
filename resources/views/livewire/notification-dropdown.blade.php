<div class="relative" x-data="{ open: @entangle('isOpen') }" @click.away="open = false; $wire.closeDropdown()"
    ...
    <button type="button" id="notification-btn" wire:click="toggleDropdown"
        ...
        @if ($this->unreadCount > 0)
            <span
                class="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white ring-2 ring-white dark:ring-gray-800">
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-200"
        ...
        class="absolute right-0 z-50 mt-2 w-80 origin-top-right rounded-lg bg-white dark:bg-gray-800 shadow-lg ring-1 ring-gray-900/5 dark:ring-gray-700 focus:outline-hidden"
        ...
        <div class="max-h-72 overflow-y-auto">
            @forelse ($this->notifications as $notification)
                <div wire:key="notif-{{ $notification->id }}"
                    class="px-4 py-3 border-b border-gray-50 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors {{ $notification->read_at ? '' : 'bg-primary-50/50 dark:bg-primary-900/10' }}">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <p
                                class="text-sm text-gray-900 dark:text-white {{ $notification->read_at ? 'font-normal' : 'font-medium' }}">
                        ...
                        @unless ($notification->read_at)
                            <button wire:click="markAsRead('{{ $notification->id }}')"
                                class="shrink-0 p-1 text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors"
