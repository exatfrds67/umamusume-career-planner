<div class="space-y-6">
        ...
        @foreach ([1 => 'Configure', 2 => 'Processing', 3 => 'Results'] as $num => $label)
            <div class="flex items-center gap-2">
                <div class="flex h-8 w-8 items-center justify-center rounded-full text-sm font-medium
                    {{ $step >= $num ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-500 dark:bg-gray-700 dark:text-gray-400' }}">
                    {{ $num }}
                </div>
                <span class="text-sm {{ $step >= $num ? 'font-medium text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400' }}">
                    {{ $label }}
                </span>
            </div>
            @if ($num < 3)
                <div class="h-0.5 w-8 {{ $step > $num ? 'bg-blue-600' : 'bg-gray-200 dark:bg-gray-700' }}"></div>
            @endif
        @endforeach
    </div>
    ...
    @if ($step === 1)
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Configure Scenarios</h2>
                <div class="flex items-center gap-2">
            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                @foreach ($scenarios as $index => $scenario)
                    <div wire:key="scenario-{{ $index }}" class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                        <h3 class="mb-3 font-medium text-gray-900 dark:text-white">Scenario {{ $index + 1 }}</h3>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="grid grid-cols-5 gap-2">
                                @foreach (['speed', 'stamina', 'power', 'guts', 'wit'] as $stat)
                                    <div>
                                        <label class="block text-center text-xs font-medium text-gray-500 dark:text-gray-400">
    ...
    @if ($step === 2)
        <div class="flex flex-col items-center justify-center py-12">
            ...
            <p class="mt-4 text-gray-600 dark:text-gray-400">Running {{ count($scenarios) }} scenarios...</p>
        </div>
    @endif
    ...
    @if ($step === 3 && !empty($report))
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Comparison Results</h2>
            ...
            @if (!empty($report['stat_comparison']))
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead>
                            <tr>
                                <th class="px-4 py-2 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Stat</th>
                                @foreach (array_keys($results) as $idx)
                                    <th class="px-4 py-2 text-center text-xs font-medium uppercase text-gray-500 dark:text-gray-400
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach ($report['stat_comparison'] as $stat => $values)
                                <tr>
                                    <td class="px-4 py-2 text-sm font-medium text-gray-900 dark:text-white">{{ ucfirst($stat) }}</td>
            ...
            @if (!empty($report['recommendations']))
                <div class="space-y-3">
                    <h3 class="font-medium text-gray-900 dark:text-white">Recommendations</h3>
                    @foreach ($report['recommendations'] as $idx => $recommendation)
                        <div class="rounded-lg bg-gray-50 p-3 dark:bg-gray-800">
                            <p class="text-sm">
                                <span class="font-medium text-gray-900 dark:text-white">Scenario {{ $idx + 1 }}:</span>
