...
<div {{ $attributes->merge(['class' => 'inline-flex flex-col gap-2']) }}>
    ...
    <div class="flex items-center gap-2 {{ $getSizeClasses() }}">
        @if ($showIcon)
            <div
                class="flex-shrink-0 w-8 h-8 rounded-full {{ $isExceeded() ? 'bg-red-100 dark:bg-red-900/30' : ($isLow() ? 'bg-amber-100 dark:bg-amber-900/30' : 'bg-green-100 dark:bg-green-900/30') }} flex items-center justify-center">
