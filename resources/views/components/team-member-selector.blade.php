...
@push('scripts')
    <script>
        function teamMemberSelector(characterId, maxTeammates) {
            return {
                ...
                init() {
                    this.loadAvailableTeammates();

                    // Listen for teammate selection changes
                    this.$watch('selectedTeammates', (value) => {
                        this.$dispatch('teammates-changed', {
                            teammates: value,
                            count: value.length,
                            unityBonus: this.getUnityBonus()
                        });
                    });
                },

                async loadAvailableTeammates() {
                    this.loading = true;
                    try {
                        // Mock data for now - replace with actual API call
                        this.availableTeammates = [{
                                id: 1,
                                name: 'Special Week',
                                specialty: 'Speed',
                                icon: '🏃',
                ...
                addTeammate(teammate) {
                    if (this.selectedTeammates.length >= this.maxTeammates) {
                        return;
                    }

                    if (!this.selectedTeammates.find(t => t.id === teammate.id)) {
                        this.selectedTeammates.push(teammate);
                    }
                },

                removeTeammate(teammateId) {
                    this.selectedTeammates = this.selectedTeammates.filter(t => t.id !== teammateId);
                },

                getUnityBonus() {
                    const count = this.selectedTeammates.length;
                    if (count === 3) return 3;
                    if (count === 2) return 2;
                    return 0;
                }
