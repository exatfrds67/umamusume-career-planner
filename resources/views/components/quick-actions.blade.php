...
<div class="fixed {{ $positionClass }} z-40" x-data="quickActions({{ json_encode($items) }})" @click.outside="close()">
    ...
    <div class="flex flex-col items-end gap-3" x-show="isOpen" x-transition>
        ...
        <template x-for="(item, index) in items" :key="index">
            <div class="flex items-center gap-2 animate-fade-in-up" :style="`animation-delay: ${index * 50}ms`">
                {{-- Label --}}
                <div
                    class="bg-neutral-800 dark:bg-neutral-700 text-white text-xs font-medium px-3 py-2 rounded-lg whitespace-nowrap shadow-lg">
                    <span x-text="item.label"></span>
                </div>

                {{-- Action Button --}}
                <button @click="executeAction(item)" :class="getItemColor(item)"
                    class="w-12 h-12 rounded-full flex items-center justify-center text-lg shadow-lg transition-transform duration-200 hover:scale-110 focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:focus:ring-offset-neutral-900"
    ...
    <button @click="toggle()" :class="isOpen ? 'scale-110 rotate-45' : 'scale-100 rotate-0'"
        class="{{ $color }} w-14 h-14 rounded-full flex items-center justify-center text-2xl shadow-xl transition-all duration-300 hover:shadow-2xl focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:focus:ring-offset-neutral-900"
