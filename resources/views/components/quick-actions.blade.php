...
<div class="fixed {{ $positionClass }} z-40"
    x-data="quickActions({{ json_encode($items) }})"
    @click.outside="close()">
    ...
    <div class="flex flex-col items-end gap-3" x-show="isOpen" x-transition>
        ...
        <template x-for="(item, index) in items" :key="index">
            <div class="flex items-center gap-2 animate-fade-in-up"
                ...
                {{-- Label --}}
                <div class="bg-gray-800 dark:bg-gray-700 text-white text-xs font-medium px-3 py-2 rounded-lg whitespace-nowrap shadow-lg">
                    <span x-text="item.label"></span>
                </div>

                {{-- Action Button --}}
                <button @click="executeAction(item)"
    ...
    <button @click="toggle()"
        :class="isOpen ? 'scale-110 rotate-45' : 'scale-100 rotate-0'"
        ...
        :aria-label="isOpen ? 'Close menu' : primaryLabel"
        :title="primaryLabel">
...
@push('scripts')
<script>
    window.Alpine && Alpine.data('quickActions', function(items) {
        return {
            items: items || [],
            isOpen: false,
            ...
            executeAction(item) {
                ...
                if (window[item.action] && typeof window[item.action] === 'function') {
                    window[item.action]();
                }
            },
            ...
            getItemColor(item) {
                const colors = {
                    'training': 'bg-green-500 hover:bg-green-600 dark:bg-green-600 dark:hover:bg-green-700',
                    ...
                    'settings': 'bg-gray-500 hover:bg-gray-600 dark:bg-gray-600 dark:hover:bg-gray-700',
                };
                
                return item.color || colors[item.type] || 'bg-blue-500 hover:bg-blue-600 dark:bg-blue-600 dark:hover:bg-blue-700';
            }
        };
    });
</script>
