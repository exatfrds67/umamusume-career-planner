...
<div
    {{ $attributes->merge([
        'class' => 'relative rounded-lg border-2 transition-all duration-200 ' . $getSizeClasses() . ' ' .
            ...
            ($card ? 'border-blue-500 dark:border-blue-400 bg-linear-to-br from-blue-50 to-blue-100 dark:from-blue-950 dark:to-blue-900' : 'border-dashed border-neutral-300 dark:border-neutral-600 bg-neutral-50 dark:bg-neutral-800 hover:border-blue-400 dark:hover:border-blue-500')
    ]) }}
    role="button"
...
>
    @if($card)
        ...
        <div class="absolute inset-0 p-2 flex flex-col">
            ...
            <div class="flex items-center justify-between mb-1">
                <span class="px-2 py-0.5 text-xs font-semibold rounded-full
                    {{ match($card['type'] ?? 'Speed') {
                        ...
                        default => 'bg-neutral-500/20 text-neutral-600 dark:bg-neutral-500/30'
                    } }}">
                    {{ $card['type'] ?? 'Speed' }}
                </span>
                ...
                @if(isset($card['limit_break']))
                    <div class="flex gap-0.5">
                        @for($i = 0; $i < 4; $i++)
                            <svg class="w-2 h-2 {{ $i < $card['limit_break'] ? 'text-yellow-400' : 'text-neutral-300 dark:text-neutral-600' }}" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="flex-1 flex items-center justify-center text-center">
                <p class="text-xs font-semibold text-neutral-900 dark:text-neutral-100 line-clamp-2">
            ...
            @if(isset($card['bond_level']))
                <div class="mt-auto">
                    <div class="flex items-center justify-between text-xs mb-1">
                        <span class="text-neutral-600 dark:text-neutral-400">Bond</span>
                        <span class="font-semibold {{ $card['bond_level'] >= 80 ? 'text-pink-600 dark:text-pink-400' : 'text-neutral-700 dark:text-neutral-300' }}">
                            {{ $card['bond_level'] }}%
                        </span>
                    </div>
    ...
    @else
        ...
        <div class="absolute inset-0 flex flex-col items-center justify-center p-2">
            <svg class="w-8 h-8 text-neutral-500 dark:text-neutral-400 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
