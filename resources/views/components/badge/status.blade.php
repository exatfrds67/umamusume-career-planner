...
@php
    ...
    $iconType = match ($statusLower) {
        'success', 'completed', 'active', 'online', 'on_track' => 'check',
        'warning', 'pending', 'at_risk' => 'warning',
        'error', 'failed', 'offline', 'off_track' => 'error',
...
<span {{ $attributes->merge(['class' => $classes]) }} aria-label="{{ $ariaLabelText }}">
    @if ($iconType === 'check')
        <svg class="w-3 h-3 shrink-0" aria-hidden="true" viewBox="0 0 20 20" fill="currentColor">
