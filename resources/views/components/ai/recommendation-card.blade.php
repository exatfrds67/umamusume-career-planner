                ...
                <div class="flex items-center gap-3 text-sm">
                    ...
                    @if (isset($recommendation->confidence_score) && $recommendation->confidence_score)
                        <span class="text-neutral-600 dark:text-neutral-400 flex items-center gap-1">
                            ...
                            <span>{{ number_format($recommendation->confidence_score * 100) }}%</span>
                            <div class="w-16 h-1.5 bg-neutral-200 dark:bg-neutral-700 rounded-full overflow-hidden ml-1">
                                <div class="h-full bg-{{ $color }}-500 rounded-full" style="width: {{ number_format($recommendation->confidence_score * 100) }}%"></div>
                            </div>
                        </span>
                    @endif
                </div>
        ...
        <div class="p-4 space-y-4">
            ...
            @if ($showActions)
                <div class="flex items-center gap-2 pt-2 border-t border-neutral-200 dark:border-neutral-700">
                    ...
                    <button type="button"
                        @click="window.dispatchEvent(new CustomEvent('send-quick-message', { detail: { message: 'Tell me more about this recommendation: {{ addslashes($recommendation->action) }}' } }))"
                        class="inline-flex items-center px-4 py-2 rounded-lg font-semibold text-sm bg-primary-100 text-primary-700 hover:bg-primary-200 dark:bg-primary-900/30 dark:text-primary-300 dark:hover:bg-primary-900/50 focus:outline-hidden focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:focus:ring-offset-neutral-900 transition-colors duration-200"
                        aria-label="Ask AI for more details about this recommendation">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            aria-hidden="true">
