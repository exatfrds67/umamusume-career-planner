...
<div class="group cursor-pointer"
    @click="$dispatch('card-selected', { id: '{{ $card['id'] ?? '' }}' })"
    role="button"
    :aria-pressed="selected"
    tabindex="0"
    @keydown.enter="$dispatch('card-selected', { id: '{{ $card['id'] ?? '' }}' })"
    ...
    <div class="{{ $dimensions }} relative rounded-lg overflow-hidden shadow-md transition-all duration-200"
        :class="selected ? 'ring-2 ring-blue-500 dark:ring-blue-400 shadow-lg scale-105' : 'hover:shadow-lg hover:scale-102 group-hover:ring-2 group-hover:ring-blue-300 dark:group-hover:ring-blue-600'">
        ...
        @else
            <div class="w-full h-full bg-gradient-to-br from-blue-400 to-purple-500 dark:from-blue-600 dark:to-purple-700 flex items-center justify-center text-2xl opacity-80">
                🎴
            </div>
        @endif

        {{-- Rarity Badge --}}
        <div class="absolute top-1 left-1 {{ $rarityClass }} text-white {{ $fontSize }} font-bold px-1.5 rounded-sm shadow-md">
            ★
        </div>

        {{-- Card Stats Overlay --}}
        <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent flex flex-col justify-end p-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
            {{-- Card Name --}}
        ...
        <div class="absolute top-2 right-2 w-5 h-5 rounded-full border-2 border-white shadow-md flex items-center justify-center transition-all duration-200"
            ...
            <svg class="w-3 h-3 text-white" :class="selected ? 'opacity-100' : 'opacity-0'" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
