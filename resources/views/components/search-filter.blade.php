...
>
    ...
    <div class="relative">
        <input
            ...
            class="w-full px-4 py-2.5 pl-10 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-hidden focus:ring-2 focus:ring-blue-500"
    ...
    <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-4">
        <!-- Character Filter -->
        <div>
            <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-2">Character</h3>
            <div class="flex flex-wrap gap-2">
                <template x-for="char in availableCharacters.slice(0, 8)" :key="char">
                    <button
                        @click="toggleFilter('character', char)"
                        :class="selectedFilters.character.includes(char) ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white'"
        ...
        <!-- Scenario Filter -->
        <div>
            <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-2">Scenario</h3>
            <div class="flex flex-wrap gap-2">
                <template x-for="scenario in availableScenarios" :key="scenario">
                    <button
                        @click="toggleFilter('scenario', scenario)"
                        :class="selectedFilters.scenario.includes(scenario) ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white'"
                        class="px-2.5 py-1 rounded-full text-xs font-medium transition hover:opacity-80"
                        x-text="scenario"
                    ></button>
                </template>
            ...
            <div class="flex flex-wrap gap-2">
                <template x-for="status in availableStatuses" :key="status">
                    <button
                        ...
                        :class="selectedFilters.status.includes(status) ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white'"
            ...
            <div class="flex flex-wrap gap-2">
                <template x-for="tag in availableTags.slice(0, 6)" :key="tag">
                    <button
                        ...
                        :class="selectedFilters.tags.includes(tag) ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white'"
                        class="px-2.5 py-1 rounded-full text-xs font-medium transition hover:opacity-80"
                        x-text="'#' + tag"
                    ></button>
                </template>
    ...
    <div class="flex gap-2">
        <button
            ...
            class="px-3 py-1.5 text-sm bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white rounded-lg transition"
        >
        ...
        <button
            ...
            class="px-3 py-1.5 text-sm bg-green-600 hover:bg-green-700 disabled:bg-gray-400 text-white rounded-lg transition"
        >
            <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @if ($showSearchHistory)
        <div x-show="searchHistory.length > 0" class="pt-2 border-t border-gray-200 dark:border-gray-700">
            <h4 class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase mb-2">Recent Searches</h4>
            <div class="flex flex-wrap gap-2">
                <template x-for="search in searchHistory" :key="search">
                    <button
                        @click="searchQuery = search; $dispatch('search-updated')"
                        class="px-2 py-1 text-xs bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white rounded hover:bg-gray-300 dark:hover:bg-gray-600 transition"
                        x-text="search"
                    ></button>
                </template>
            </div>
    ...
    <!-- Saved Filters -->
    @if ($showSavedFilters)
        <div x-show="savedFilters.length > 0" class="pt-2 border-t border-gray-200 dark:border-gray-700">
            <h4 class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase mb-2">Saved Filters</h4>
            <div class="space-y-1">
                <template x-for="filter in savedFilters" :key="filter.id">
                    <div class="flex items-center justify-between gap-2 p-2 bg-gray-50 dark:bg-gray-800 rounded">
                        <button
                            ...
                            class="flex-1 text-left text-sm text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition"
                            x-text="filter.name"
                        ></button>
                        <button
