                ...
                @else
                    <ul class="grid grid-cols-1 md:grid-cols-2 gap-4" role="list">
                        @foreach($races as $race)
                            <li role="listitem">
                                <a href="{{ route('races.show', $race) }}" class="block card bg-white dark:bg-gray-800 hover:ring-2 hover:ring-primary-500 focus:ring-2 focus:ring-primary-500 focus:outline-none transition-all cursor-pointer group h-full">
                                    <article class="card-body h-full flex flex-col">
                                        <div class="flex justify-between items-start mb-2">
                                            <div>
                                                <div class="flex items-center gap-2 mb-1">
                                                    <span class="badge 
                                                        {{ $race->race_grade === 'G1' ? 'badge-warning' : 
                                        ...
                                        <div class="flex items-center gap-3 text-xs text-gray-500 mt-auto pt-4">
                                            @if($race->weather)
                                                <span class="flex items-center gap-1" title="Weather: {{ $race->weather }}">
                                            ...
                                            @endif
                                            @if($race->character)
                                                <span class="flex items-center gap-1" title="Character: {{ $race->character->name }}">
                                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
