    ...
    <div class="space-y-6">
            ...
            <div class="md:flex">
                <div class="p-8 md:w-1/2 bg-linear-to-br from-primary-600 to-primary-800 text-white flex flex-col justify-center">
                    ...
                    <div class="mt-1 text-xl text-primary-100 flex items-center gap-2">
                         ...
                         <span>•</span>
                         <span>{{ $race->distance_category }}</span>
                         <span>•</span>
                         <span>{{ $race->weather ?? 'Unknown Weather' }}</span>
                    </div>
                    @if($race->race_conditions)
                        <div class="mt-6">
                            <h4 class="text-sm font-bold text-primary-200 uppercase">Conditions</h4>
                    ...
                    @if($race->character)
                        ...
                        <div class="flex items-center gap-4 mb-6">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500">Condition: {{ $race->character_condition ?? 'Unknown' }} | Mood: {{ $race->motivation ?? 'Normal' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-5 gap-2 text-center">
                            @foreach(['speed', 'stamina', 'power', 'guts', 'wit'] as $stat)
                                ...
                                <div class="bg-gray-50 dark:bg-gray-700 rounded p-2">
                                    <div class="text-xs text-gray-500 uppercase">{{ $stat }}</div>
                                    <div class="font-bold text-gray-900 dark:text-white">{{ $val }}</div>
                                </div>
                            @endforeach
                        </div>
                    @else
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            @if($race->preparation_strategy)
                <div class="card bg-white dark:bg-gray-800 h-full">
                    <div class="card-header">
                        <h3 class="font-medium">Preparation Strategy</h3>
                    </div>
                    <div class="card-body">
                 ...
                 <div class="card bg-white dark:bg-gray-800 h-full">
                    ...
                    <div class="card-body">
                         <div class="prose dark:prose-invert text-sm">
                             {{-- Assuming string or structured data, handle gracefully --}}
