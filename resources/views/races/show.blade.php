    ...
    <div class="space-y-6">
        ...
        @php
            $gradeColors = [
                ...
                'Debut'  => 'bg-neutral-100 text-neutral-700 dark:bg-neutral-700 dark:text-neutral-300',
            ];
            $phaseLabels = [
        ...
        <header class="flex flex-wrap items-start justify-between gap-4">
            <div>
                <div class="flex flex-wrap items-center gap-2 mb-1">
                    <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-bold {{ $gradeColors[$gameRace->grade] ?? 'bg-neutral-100 text-neutral-700 dark:bg-neutral-700 dark:text-neutral-300' }}">
                    ...
                    @if (isset($phaseLabels[$gameRace->phase]))
                        <span class="text-xs text-neutral-500 dark:text-neutral-400">{{ $phaseLabels[$gameRace->phase] }}</span>
                    @endif
                </div>
                <h1 class="text-2xl font-bold text-neutral-900 dark:text-white">{{ $gameRace->name_en }}</h1>
                @if ($gameRace->name_jp)
                    <p class="mt-0.5 text-sm text-neutral-500 dark:text-neutral-400">{{ $gameRace->name_jp }}</p>
                @endif
            </div>
        ...
        {{-- Course Info Card --}}
        <section class="card bg-white dark:bg-neutral-800 overflow-hidden" aria-labelledby="course-info-heading">
            ...
            <div class="md:flex">
                ...
                <div class="md:w-3/5 p-8 grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {{-- Fan Requirement --}}
                    <div>
                        <h3 class="text-xs font-semibold uppercase tracking-wide text-neutral-500 dark:text-neutral-400 mb-2">Fan Requirement</h3>
                        @if ($gameRace->fan_requirement)
                            <p class="text-2xl font-bold text-neutral-900 dark:text-white">
                                {{ number_format($gameRace->fan_requirement) }}
                                <span class="text-sm font-normal text-neutral-500 dark:text-neutral-400">fans</span>
                            </p>
                        @else
                            <p class="text-sm text-neutral-400 dark:text-neutral-500">None</p>
                        @endif
                    </div>

                    {{-- Stat Requirements --}}
                    <div>
                        <h3 class="text-xs font-semibold uppercase tracking-wide text-neutral-500 dark:text-neutral-400 mb-2">Stat Requirements</h3>
                            ...
                            <dl class="space-y-1">
                                @foreach ($gameRace->stat_requirements as $stat => $value)
                                    <div class="flex items-center justify-between text-sm">
                                        <dt class="capitalize text-neutral-600 dark:text-neutral-400">{{ $stat }}</dt>
                                        <dd class="font-semibold text-neutral-900 dark:text-white">{{ number_format($value) }}</dd>
                                    </div>
                                @endforeach
                            </dl>
                        @else
                            <p class="text-sm text-neutral-400 dark:text-neutral-500">None recorded</p>
                        @endif
                    </div>

                    {{-- Fans Reward --}}
                    <div>
                        <h3 class="text-xs font-semibold uppercase tracking-wide text-neutral-500 dark:text-neutral-400 mb-2">Fans Reward (Win)</h3>
                    ...
                    {{-- SP Reward --}}
                    <div>
                        <h3 class="text-xs font-semibold uppercase tracking-wide text-neutral-500 dark:text-neutral-400 mb-2">SP Reward</h3>
                        @if ($gameRace->sp_reward)
                            <p class="text-2xl font-bold text-neutral-900 dark:text-white">
                                +{{ number_format($gameRace->sp_reward) }}
                                <span class="text-sm font-normal text-neutral-500 dark:text-neutral-400">SP</span>
                            </p>
                        @else
                            <p class="text-sm text-neutral-400 dark:text-neutral-500">—</p>
                        @endif
        ...
        @if ($gameRace->year_in_scenario)
            <section class="card bg-white dark:bg-neutral-800 p-6" aria-labelledby="scenario-heading">
                <h2 id="scenario-heading" class="text-sm font-semibold uppercase tracking-wide text-neutral-500 dark:text-neutral-400 mb-2">Scenario Timing</h2>
        ...
        <div class="flex items-center gap-4">
            ...
            @auth
                @php
                    $activeCharacters = \App\Models\Character::query()
                ...
                @if ($activeCharacters->isNotEmpty())
                    <div x-data="{ open: false }" class="relative">
                        <button @click="open = !open" type="button" class="btn btn-primary focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2 rounded">
                            🏇 Enter Race with Character
                            <svg class="ml-1 h-4 w-4 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
                        </button>
                        <div x-show="open" @click.outside="open = false" x-transition class="absolute z-10 mt-2 w-64 rounded-md bg-white dark:bg-neutral-800 shadow-lg ring-1 ring-black/5 dark:ring-white/10">
                            <div class="py-1">
                                @foreach ($activeCharacters as $activeChar)
                                    <form action="{{ route('races.enter', [$activeChar, $gameRace]) }}" method="POST" class="block">
                                        @csrf
                                        <button type="submit" class="w-full text-left px-4 py-2 text-sm text-neutral-700 dark:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-700">
                                            {{ $activeChar->name }}
                                            <span class="text-xs text-neutral-400 dark:text-neutral-500 ml-1">Turn {{ $activeChar->current_turn }}</span>
                                        </button>
                                    </form>
                                @endforeach
                            </div>
